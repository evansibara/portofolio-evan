/**
 * GradualBlur — React Bits component
 ...
 *  1. CSS di-inline di dalam file ini (tidak perlu file GradualBlur.css terpisah).
 *  2. Dependency `mathjs` DIHAPUS — diganti Math native JavaScript.
 *     Kode asli hanya pakai math.pow (= Math.pow) dan math.round (= Math.round),
 *     keduanya sudah built-in, jadi tidak perlu library 700KB.
 */
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      if (config.exponential) {
        // Math.pow native — sebelumnya math.pow dari mathjs
        blurValue = Math.pow(2, progress * 4) * 0.0625 * currentStrength;
      } else {
        blurValue = 0.0625 * (progress * config.divCount + 1) * currentStrength;
      }

      // Math.round native — sebelumnya math.round dari mathjs
